...
public class enemy_sprite : MonoBehaviour {
	...
	void Update ()
	{
		...
		if (return_init == true)
		{
			...
			{
				transform.LookAt (new Vector2 (initial_pos, -7f));
		...
		if(aggro == true && !(Player.transform.position.x >= distance_left && Player.transform.position.x <= distance_right))
		{
			...
			if (distance > 5)//transform.position.x >= max_follow_right || transform.position.x <= max_follow_left)
			{
				aggro = false;
				return_init = true;
	...
	void TurnToPlayer()
	{
		...
		transform.LookAt (new Vector2 (Player.transform.position.x, -7f));
